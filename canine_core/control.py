...
def filtered_aliases(cfg: CanineConfig) -> List[str]:
    al = list(ALIASES)
    # Hide voice-dependent behaviors when voice is disabled
    if not bool(getattr(cfg, "ENABLE_VOICE_COMMANDS", True)):
        al = [a for a in al if a not in ("voice_patrol", "whisper_voice_control")]
    return al
...
def main() -> None:
    ...
    # Use default config instance to filter menu items where needed
    cfg = CanineConfig()

    if mode_idx == 0:  # single
        options = filtered_aliases(cfg)
        idx = prompt_menu("Pick a module to run once:", options)
        spec = options[idx]
        ...
        indices = [int(x)-1 for x in raw.split(',') if x.strip().isdigit() and 1 <= int(x) <= len(options)]
        seq = [options[i] for i in indices] or ["idle_behavior"]
        ...
        print(", ".join(options))
        asyncio.run(orch.run_random_cycle(options, float(dur_min), float(dur_max)))
        ...
        # Filter preset choices based on voice flag as a safety
        choices = getattr(cfg, 'AVAILABLE_BEHAVIORS', ALIASES)
        if not bool(getattr(cfg, "ENABLE_VOICE_COMMANDS", True)):
            choices = [c for c in choices if c not in ("voice_patrol", "whisper_voice_control")]
